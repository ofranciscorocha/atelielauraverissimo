...
export default function CheckoutPage() {
  ...
  const finalTotal = subtotal + (currentShipping?.price || 0);
  ...
  const handleCalculateShipping = async () => {
    ...
    try {
      ...
      if (result.success && result.options) {
        ...
        alert(result.error || 'Erro ao calcular frete');
        // Fallback para opções estáticas
        setShippingOptions([
          { id: 'sedex', name: 'SEDEX', price: 25, deliveryTime: '3-5 dias úteis', company: 'Correios' },
          { id: 'pac', name: 'PAC', price: 15, deliveryTime: '7-12 dias úteis', company: 'Correios' }
        ]);
